...
function _assignEsc(value) {
  if (typeof _wsEsc === 'function') return _wsEsc(value);
...
function _renderAssignPopup() {
  ...
  pop.innerHTML = `
    <div style="width:100%;max-width:480px;background:#fff;border-radius:24px 24px 0 0;max-height:85vh;overflow:hidden;display:flex;flex-direction:column;" data-no-context-menu>
      ...
      <div style="flex:1;padding:12px 16px;overflow:hidden;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
          ...
          <button data-assign-add style="padding:5px 10px;border-radius:6px;border:1.5px solid var(--accent);background:transparent;color:var(--accent);font-size:11px;font-weight:700;cursor:pointer;">+ 추가</button>
        </div>
      ...
      ${_selectedIds.size > 0 ? `
        <div style="padding:10px 16px;border-top:1px solid var(--border);background:#fff;display:flex;align-items:center;justify-content:space-between;">
          ...
          <button data-delete-selected style="padding:8px 14px;border-radius:8px;border:1px solid rgba(220,53,69,0.4);background:transparent;color:var(--danger);font-size:11px;font-weight:600;cursor:pointer;">삭제</button>
        </div>
      ` : ''}
    </div>
  `;
  pop.querySelector('[data-no-context-menu]')?.addEventListener('contextmenu', e => e.preventDefault());
  pop.querySelector('[data-assign-close]')?.addEventListener('click', closeAssignPopup);
  pop.querySelector('[data-assign-add]')?.addEventListener('click', _addSlotInPopup);
  pop.querySelector('[data-delete-selected]')?.addEventListener('click', _deleteSelectedInPopup);
  pop.querySelectorAll('[data-assign-slot]').forEach(el => {
    el.addEventListener('click', () => _assignToSlotFromPopup(el.dataset.assignSlot));
  });
  pop.querySelectorAll('[data-delete-slot]').forEach(btn => {
    btn.addEventListener('click', event => {
      event.stopPropagation();
      _deleteSlotInPopup(btn.dataset.deleteSlot);
    });
  });
  pop.querySelectorAll('[data-photo-select]').forEach(el => {
    el.addEventListener('click', () => {
      togglePhotoSelect(el.dataset.photoSelect);
      _renderAssignPopup();
    });
  });
}
