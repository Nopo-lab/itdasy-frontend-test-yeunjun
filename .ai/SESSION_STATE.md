...
**LAST UPDATED:** 2026-09-23 · T-915 private 이미지 임시 주소 자동 갱신 구현·검증, 배포 전 PR 대기

**PREVIOUS UPDATE:** 2026-09-16 · T-904 실제 출시 인증 추가 실행·수정·배포 완료, 출시 보류

---

## LAST CHECKPOINT — 2026-09-23 KST · T-915

- 별도 작업 폴더: 프론트 `../itdasy-signed-url-fe`, 서버 `../itdasy-signed-url-be`, 브랜치 `codex/T915-signed-url-refresh`.
- Supabase 이미지 저장소는 private 유지. 앱이 켜져 있고 원장이 작업 중이면 보이는 이미지의 임시 주소를 서버에서 새로 받아 자동 교체한다.
- 이미지 로드 실패 때도 회색 실패 화면 전에 1회 새 임시 주소를 먼저 요청한다.
- 서버는 로그인한 사용자 자기 폴더 URL만 새로 발급한다. 외부 URL과 다른 사용자 폴더 URL은 거부한다.
- 검증: 프론트 관련 16개 통과, 관련 자동 검사 오류 0. 서버 private storage/새 주소 발급 12개 통과.
- 운영 배포와 실제 앱 스토어 재제출은 하지 않음.
