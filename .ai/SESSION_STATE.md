...
**LAST UPDATED:** 2026-09-16 · T-904 실제 출시 인증 실행 및 수정 완료, 출시 보류
...
- 별도 작업 폴더: `../itdasy-release-cert-fe`, `../itdasy-release-cert-be`, 가지 `cert/T-904-absolute-release`.
- 최종 판정: RELEASE HOLD. 게이트 PASS 6 / FAIL 7 / NOT VERIFIED 10 / BLOCKED 2.
- FE 후보 `89ee44d`: AI 동의를 최초 요청 계정에 고정해 계정 전환 중 다른 계정 저장·옛 요청 재시도를 막음. Instagram 연결 전 자동 동의 기록 제거.
- BE 후보 `3690929`: 외부 AI 진입점 31곳을 현재 동의로 막고 DM 재생성 우회를 닫음. Instagram 분석 동의를 전용 2.0으로 분리해 과거 자동 1.0 기록을 거절. GitHub 격리 DB 주소 오판과 시험 사용자 번호 충돌도 수정.
- 전체 검사: FE 206묶음 3,220개 성공. BE 로컬 4,591개 성공·291개 미실행·예상 실패 1개. 변경안 자동 검사는 일반 4,586개, 순서 섞기 4,585개, PostgreSQL 211개와 매장 분리·FE 연결값까지 다섯 항목 모두 성공.
- 실제 브라우저: 같은 계정은 동의 저장 1회→재시도 1회→200. 계정 전환은 저장 0회→재시도 0회→400. 모바일 가입 링크·약관 영역 44px 실측.
- 격리 DB: 2,365 요청, 매장 20곳, 다른 매장 번호 변조 357회, 돈·회원권 규칙 15개 불일치 0. 다만 동시 흐름 뒤 90초 끊김으로 안정성·성능 실패.
- 원격 main 재확인: FE `9888cece`, BE `ff6253d`. BE 새 변경은 관측 기록 파일 2개뿐이라 제품 영향 없음. 실제 제공 BE는 계속 `9850751e`.
- 미해결: 공개 사진 저장소, Remove.bg 열쇠 미교체, 실제 정책과 삭제 주소 404, 실제 Meta·상점 결제·후보 모바일·DB 복원·12명 사람 시험·유효 사진 60장과 원장 6명 평가.
