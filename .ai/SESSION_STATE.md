...
**LAST UPDATED:** 2026-09-15 · T-602 고객 케어 구현·검증·원격 푸시 완료
...
- 작업 폴더 `../itdasy-customer-care-fe`, 브랜치 `fe/T-602-customer-care`. 서버는 `../itdasy-customer-care-be`, `be/T-602-customer-care`.
- 고객 상세에 시술 노트(추가/수정/이전 내용 복사), 다음 관리일(지정/해제), 소개 고객 연결. 고객 목록에 관리일 목록 추가.
- 입력 중 이동·닫기·자동 갱신 보호, 저장 실패 시 입력 보존, 같은 저장 요청 중복 방지.
- 서버에서 수동 관리일·해제 유지, 원장별 소개 연결 제한, 고객 합치기/삭제 처리.
