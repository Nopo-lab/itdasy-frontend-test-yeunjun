...
- 저장소 이미지: Supabase 버킷은 private 유지. 앱이 켜져 있고 원장이 작업 중이면 보이는 사진의 임시 주소를 서버에서 자동으로 새로 받아 교체한다. 이미지 로드 실패 때도 회색 오류 화면 전에 1회 새 주소를 요청한다.
...
| 인스타 **게시물 댓글 답글 자동화** | 🟡 **스테이징 ON** — `INSTAGRAM_FULL_SCOPE=1`(Cloud Run env, 코드 아님)이라 개발모드/테스터는 App Review 전에도 `manage_comments` 획득 가능. **운영은 env 없음 → basic scope**. 스코프는 동의 시점에 토큰에 박히므로 **기존 연동자는 인스타 재연결 필요**(refresh-token 으론 안 바뀜) | `instagram.py:74,710`, `/instagram/comment-queue`(:799) · FE 큐 UI `app-comment-reply-queue.js` **[2026-07-20 v785 리디자인]** 게시물칩+미리보기팝업·채널별 토글(답글만/DM만)·DM접기 + 홈 "AI 잇비가 챙겼어요" 진입줄(`app-home-v41.js _fetchCommentQueueCount`) **[2026-07-21]** 분류 인텐트 확장 = price·booking·location·hours·service·**duration(소요시간·지속력, 유실복구)·event(이벤트)·membership(회원권)·eligibility(건강여부→항상 사람)**·complaint(`instagram.py _classify_comment` + LLM 재판정 `comment_intent_llm.py`) · **응답 시간대(방해금지)** 설정 = `itdasy:crq_settings.active_hours`+`quiet_outside`, 운영시간 밖엔 홈 넛지 뮤트(`window.crqQuietNow`)+큐 배너, 발송은 언제든 가능 · autoreply_gate `within_hours`(DM 운영시간 재사용, 자동발송 켜질 때만 유효·shadow) **[2026-07-22 v790]** 설정 **서버 보관** = `GET/PUT /instagram/comment-reply-settings` → `dm_auto_reply_settings.crq_settings_json`(예전엔 localStorage 뿐이라 폰 교체·캐시 삭제로 소실). 헤더 **저장 버튼**(DM 자동응답과 같은 자리) + 세부설정 2종 = `default_dm`(새 문의의 'DM 같이 보내기' 기본값)·`exclude_words`(이 말 든 댓글은 큐에서 제외, 쉼표 구분) **[2026-07-25~26]** **'무시' 서버 영속화**(`f8b833a`) — 예전엔 로컬만이라 배지 숫자와 리스트가 어긋나고 기기 간 동기화가 안 됐음 · **webhook 저장분(`CommentAuthorLog`) 큐 병합**으로 실시간화(`9c4897c`) · **자동갱신 폴링 + 토글 라벨 정정**(`bfc1666`) · 🚨 **발송 API(답글·DM)를 전역 재시도 목록에서 제외**(`33cdd1f`) — 안 그러면 같은 답글이 최대 4번 나감 · 공개 초안에서 DM 언급 제거(`5f617ff`, 화면 = 실제로 달리는 문구)  **[2026-09-01 P0 + 하드닝]** 🔴 **운영 실측: `INSTAGRAM_FULL_SCOPE=1` 이 Cloud Run 에 실제로 걸려 있다** — 이 기능은 라이브다. 🔴 **P0: [보내기]를 눌러도 답글이 안 나가고 있었다.** FE 가 발송 때 dismiss 를 먼저 쏘고(2026-07-25), BE 가 발송 전 선점을 넣으면서(2026-08-23 `0052`+선점) 그 마커 행이 선점을 차지 → `duplicate` 조기 반환 → **Graph 호출 0회인데 화면엔 '답글 달림'**. 각각은 옳은 변경인데 **다른 레포·다른 세션**이라 교차 지점을 아무도 못 봤다. 수정: 판정 기준을 intent 가 아니라 **`public_reply_id`/`dm_sent`(실제로 나갔는가)** 로 — 나간 적 없는 마커(`_dismissed`/`_gone`)는 선점으로 안 쳐주고 **인수해서 발송**. FE 는 `_dismissItem` 분리로 애초에 안 쏜다(양쪽 다 고침). 테스트 `tests/test_comment_dismiss_blocks_reply_p0.py`(7). **그 외 재이식(upstream 에 없던 것만)**: 실시간 큐 소스를 `CommentAuthorLog`→**`CommentEvent`** (`0054` 가 적재까지 해놓고 **읽는 쪽이 요약 테이블에 남아** 같은 사람 두 번째 댓글 유실이 그대로였다) · `count_only` **진짜 저비용화**(예전엔 이름과 달리 Graph 전량 스캔 후 개수만 셌다 → 홈 배지가 외부호출 60회+Gemini 28회를 태웠다) · 큐 자격 정본 `_crq_view_settings`/`_crq_item_eligible`(배지==목록, 불만·건강여부는 설정으로 못 끔) · `_cq_drop_replied`(멀티인스턴스 캐시 stale) · `CommentDraftCache` user_id 필터 · `draft_source`(ai/template) · `_classify_ig_reply_error`→`error_code`(Graph 원문 JSON 대신 사람 말) · 삭제 댓글 `_gone` 종결(일반 500 은 삭제로 단정 안 함) · `week_replied` 는 실제로 나간 것만. **FE**: 예시(SEED) 댓글 완전 제거 + 상태 7종(LOADING/DATA/EMPTY/DISABLED/PERMISSION/NOT_CONNECTED/NETWORK) + `permission_error` 재연결 CTA(`openIntegrationsHub`) · 우선순위 **불만→건강→돈문의→일반**(좋아요 수 제거, 동급은 단골 먼저) · 안 나가는 **DM 편집칸·예약링크 설정칸 제거** · 발송 실패 시 카드 복원 · 불만·건강여부엔 설정 이모지 미부착(사과에 😊 금지) · 로컬 숨김 30일 TTL. 테스트 `__tests__/comment-queue-hardening.test.js`(25) · 문서 `COMMENT_SYSTEM_AUDIT_2026-08-31.md`·`COMMENT_P0_AND_REBASE_2026-09-01.md` ⚠️ **중복이라 폐기한 것**(이미 upstream): `0052_comment_reply_uq` UNIQUE · 선점 로직 · `0054_comment_events` 테이블·적재 · auth 탈퇴 정리 |
...
- AI 추천/생성형: ❌ **[2026-08-21 정정] 프론트 파일 없음** — reco-*·generative-* 7파일은 `64752a2`(옛 PhotoEditor 껍데기 49파일 삭제)에 포함돼 삭제됨. 백엔드 PE-AI-1B(`photo_editor_ai.py` 330줄 + `PhotoAiCache` + `photoai_daily` 한도)는 살아 있으나 **호출하는 프론트가 0** — 재배선 계획은 루트 `PHOTO_AI_ENGINE_VALIDATION_2026-08-21.md` Phase 7 참조.
...
- **js/workspace/work-memory.js**(263, `window.WorkMemory`) — **[2026-09-14 T-601] 원장 작업 기억.** 현재 작업실 V2 저장·발행 준비 뒤, 원장이 동의한 편집 스타일만 최대 10개 보관한다. 자동 적용 기본값은 **꺼짐**이며, 같은 계정·같은 업종·같은 게시 목적이고 원장이 자동 적용을 따로 켠 기록만 자동으로 적용한다. 저장 `itdasy:work_memory:list`/`:default`(localStorage).
  - **[2026-09-15 T-601] 다양성 흐름 보강.** 같은 원장 안에서도 완성샷/이벤트/전후사진/샵스타일 A/B를 따로 학습한다. 사진 수만 넓게 참고하고 게시 목적·전후 여부는 섞지 않는다. 이벤트는 문구 내용이 아니라 편집 형식만 이벤트 칸 안에서 학습한다. 잠금 = `work-memory-diverse-owner-style-flow-2026-09-15.test.js`.
  - 담는 값 = `ItdEditor._exportState()`(editState)에서 **그 사진 전용값 제거**(`photos`·`photoDraw`·`pz`·`cellCrop`) 후 재사용분(`layers`·`ratio`·`layoutIdx`·`collage*`)과 원장이 누른 뷰티 보정 `presetId`/강도만. 저장·재편집용 현재 사진 보정값은 `adj`에 남지만, 다른 사진 추천/자동 적용용 기억에는 원본 숫자를 그대로 복사하지 않는다. ⚠️ **붓 그림은 기억 못 함** — `photoDraw`는 벡터가 아니라 그 사진에 구워진 PNG.
  ...
  - 훅 4곳(호출 1줄씩, 로직 없음): `workspace-v2-flow.js` 저장·발행 준비 성공·`_markPublishedNow()`(붙잡기) + `_openStoryEditor()`(추천/자동 적용). UI = `work-memory-consent-ui.js` 동의 카드 + `workspace-settings.js` '내 샵 스타일' 설정 + 편집기 상단 `지난 ○○ 스타일 적용`/`이번 스타일 빼기`. CSS `css/screens/sub-screens.css` `wm-*`.
  - **[T-601] 다시 쓰기 — `work-memory-policy.js`가 계정·업종·목적·동의 버전을 먼저 검사한 뒤, 추천만 표시하거나 원장이 켠 경우에만 자동 적용한다.** 롤백 `?wsmem=0` · 강제 `?wsmem=1`.
    ...
    - 보정만 쓴 스타일도 기억 후보가 된다. 적용 뒤 안내가 사라져도 편집기 상단 `이번 스타일 빼기`와 되돌리기로 뺄 수 있다.
