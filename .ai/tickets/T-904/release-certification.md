...
| BE 원격 main | `9a2f314353b05246798f710fba3266a8dcae5d7b` |
| BE 실제 제공 SHA | `9a2f314353b05246798f710fba3266a8dcae5d7b` |
| Cloud Run | `itdasy-backend-staging-00649-q9r`, 트래픽 100%, `/live` 200 |
...
잘못 재시작하던 문제와 AI 동의 뒤 캡션 재개 문제를 포함한다. BE `9a2f3143`은 Cloud Run에서
100% 제공 중이며 `/live` 200과 실제 제공 환경값을 대조했다. 00629는
...
| BE 전체 자동 검사 | BE `9a2f3143` | 공통 | 실패 0 | 4,715개 성공, 299개 제외, 1개 예상 실패. 격리 PostgreSQL 214개·필수 매장분리 99개는 제외 없이 성공 | 로컬 전체 실행·격리 DB 실행 | PASS |
...
| 젤네일 캡션 업종·사실 일치 | 실제 FE·cbt4·BE `9a2f3143` | B/L | 네일 입력에 다른 업종 문구·태그, 입력 없는 완료 결과·반응, 깨진 문장 0 | 최종 배포 뒤 같은 입력 10회 모두 기준 충족. 말투 분석 24개·지문 59개 실제 적용 확인 | BE #71~#89·`evidence/live-caption-final-20260916.json` | PASS |
...
| T904-D59 | P1 AI Truthfulness | 손끝에 아름다움을 채움, 정돈 마무리, 만족스러운 결과처럼 새 표현으로 완료 결과를 단정 | 거짓 시술 결과 광고 위험 | 신체 결과 행동·마무리 결과·결과 만족 단정을 근거 없으면 제거 | BE #87~#89, 관련 88개·전체 4,715개·실제 10회 PASS |
