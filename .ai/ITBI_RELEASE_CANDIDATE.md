...
D LLM related_questions              PASS 27/27 · 노출 80(unique 62) · invalid 0
...
**런타임 재수집 — 완료 (2026-08-19)**

```
27 / 27 콜 완주
노출된 추천질문   80개 (unique 62)
invalid exposed   0
out-of-domain     0
unsupported       0
entity misparse   0
...
지난 FAIL 이던 `"다른 샵 회원권 사례 보여줘"` 계열이 **80개 중 0회**였다.
회원권 시나리오는 3 run 내내 우리 데이터로 답할 수 있는 것만 나왔다:

    run1  회원권 잔액 남은 고객은 누구야? / 회원권 충전 기록은 어떻게 해? / 회원권 사용 내역은 어디서 봐?
    run2  회원권 잔액 남은 손님 알려줘 / 회원권 충전은 어떻게 해? / 회원권 차감은 어떻게 해?
    run3  회원권 잔액 부족 고객 알려줘 / 회원권 충전·차감은 어떻게 해? / 우리 샵 단골 고객은 누구야?

쿼터 소진으로 세 번에 나눠 돌았다(9 → 7 → 5 → 6). 이어하기가 있어 재실행분 0.

**→ D LLM related_questions = PASS**
