...
(function () {
  ...
  function showUndoToast(message, logId) {
    ...
    el.innerHTML = `<span>${_esc(message)}</span>` + (logId ? `<button data-undo style="background:#BC6675;border:none;color:#fff;padding:5px 10px;border-radius:8px;font-weight:700;cursor:pointer;font-size:12px;">되돌리기</button>` : '');
  ...
  function _ensureSheet() {
    ...
    sheet.innerHTML = `
      <div id="uhsCard" style="width:100%;max-width:540px;background:#fff;border-radius:20px 20px 0 0;max-height:88vh;display:flex;flex-direction:column;padding:18px 18px max(18px,var(--safe-area-inset-bottom, env(safe-area-inset-bottom, 0px)));">
        ...
        <div style="margin-top:10px;padding:12px;background:#F7EFF0;border-radius:10px;font-size:11px;line-height:1.5;color:#BC6675;">
    ...
    try { window._bindSheetBack && window._bindSheetBack('assistantundo', sheet, () => { sheet.remove(); }); } catch (_bsb) { void _bsb; }
