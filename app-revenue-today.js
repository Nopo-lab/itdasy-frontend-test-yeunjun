/* [매출감사 2026-08-04] ⚠️ 이 파일은 **현재 도달하지 않는다.**
     app-revenue.js:1023 이 호출하지만 그 위 `if (_currentPeriod === 'month') return` 아래라서
     day/week 일 때만 온다. 그런데 period 전환 UI(data-rv-act="period")가 저장소 어디에도
     렌더되지 않아 _currentPeriod 는 'month' 로 고정이다.

     지우지 않은 이유: 일/주 보기는 되살릴 수 있는 화면이고(코드가 완성돼 있다),
     지우면 app-revenue.js 의 분기까지 손대야 한다. 지금 이 파일은 **비용이 0**이다
     — lazy 그룹에 있고 실행되지 않는다. 되살릴 땐 전환 버튼 마크업만 넣으면 된다. */
/* ─────────────────────────────────────────────────────────────
