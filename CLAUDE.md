...
매 배포마다 `index.html`·`js/load-groups.js`·`style.css` 의 **모든 `?v=` 를 빌드 버전으로
통일**한다 (`scripts/bump_cache_busters.py`, 실측 332건). `load-groups.js` 자신의 버전과
`style.css` 의 `@import` 자식 6개도 포함되며, 둘 중 하나라도 안 바뀌면 배포를 **실패시킨다** —
그거 하나 빠지면 나머지가 전부 무효이기 때문.
...
- ✅ **`style.css` 의 `@import` 도 자동이다** (2026-09-12 부터). 그전까진 `TARGETS` 밖이라
  자식 6개의 `?v=` 가 **얼어붙어 있었다** — 실측으로 `style-dark.css` 가 `?v=20260520-v242`,
  약 4개월째 고정이었다.
  🔑 이게 왜 가장 속기 쉬운 형태였나: `index.html` 의 `style.css?v=` 는 자동으로 올라가니
  브라우저가 **style.css 는 새로 받는다.** 그런데 그 안의 자식 URL 이 옛 `?v=` 라
  **자식 CSS 는 캐시에서 꺼내 쓴다.** 겉보기엔 버스팅이 도는데 스타일만 안 바뀐다.
  이제 자식이 안 바뀌면 배포가 **실패**한다(네거티브 테스트로 실증).
