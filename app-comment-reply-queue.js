...
(function () {
  ...
  function _errorMessage(j) {
    ...
    /* [종결게이트 2026-09-07] 인스타 응답이 유실된 경우는 **보냈는지 우리도 모른다.**
       "보내지 못했어요" 라고 하면 모르는 걸 단정하는 것이고, 원장님이 손님에게
       다르게 말하게 된다. 카드는 큐에 남겨두고, 잠시 뒤 다시 누르면 서버가
       인스타에 직접 물어봐서(이미 달렸으면 발송 생략) 정정해 준다. */
    if (code === 'unverified') return '전송 결과를 확인하지 못했어요 — 잠시 후 다시 눌러 확인해 주세요';
