...
(function () {
  ...
  function _ensureSheet() {
    ...
    sheet.innerHTML = `
      <div style="position:absolute;inset:auto 0 0 0;background:var(--bg,#fff);border-radius:20px 20px 0 0;max-height:92vh;display:flex;flex-direction:column;padding:16px;padding-bottom:max(16px,var(--safe-area-inset-bottom, env(safe-area-inset-bottom, 0px)));">
  ...
  // 이탈 예측·매출 예측은 매출/예약 데이터에 전부 영향 받음
  if (typeof window !== 'undefined' && !window._insightsDataListenerInit) {
    ...
    window.addEventListener('itdasy:data-changed', async (e) => {
      ...
      if (!k) return;
      // 인사이트에 영향 주는 kind: 매출·예약·고객·지출 전부
      const affects = ['create_revenue', 'update_revenue', 'create_booking', 'update_booking',
                       'cancel_booking', 'reschedule_booking', 'create_customer',
